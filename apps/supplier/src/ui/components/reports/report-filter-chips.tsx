import { Calendar, ChevronDown, ListFilter, SlidersHorizontal, Tag, Package, Users, FileCheck } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@energyiq/ui';

export interface ReportFilter {
  id: string;
  label: string;
  options: string[];
}

export type ReportFilterSelection = Record<string, string | null>;

const FILTER_ICON: Record<string, LucideIcon> = {
  products: Package,
  type: Tag,
  distributor: Users,
  status: FileCheck,
  'from-date': Calendar,
  'to-date': Calendar,
  tier: Tag,
  region: Users,
  default: ListFilter,
};

interface ReportFilterChipProps extends ReportFilter {
  selected: string | null;
  onSelect: (option: string | null) => void;
}

function ReportFilterChip({ id, label, options, selected, onSelect }: ReportFilterChipProps) {
  const LeadingIcon = FILTER_ICON[id] ?? FILTER_ICON.default;

  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="tap-effect flex items-center gap-1.5 rounded-[14px] border-[1.5px] border-[#616161B2] px-3 py-1 text-xs text-white"
        >
          <LeadingIcon className="h-3.5 w-3.5 text-[#FBC02D]" aria-hidden="true" />
          {selected ?? label}
          <ChevronDown className="h-3.5 w-3.5 text-[#FBC02D]" aria-hidden="true" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start" className="min-w-[160px]">
        {options.map((option) => (
          <DropdownMenuItem
            key={option}
            onSelect={() => onSelect(option === selected ? null : option)}
            className={option === selected ? 'text-[#FBC02D]' : undefined}
          >
            {option}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}

interface ReportFilterChipsProps {
  filters: ReportFilter[];
  selection: ReportFilterSelection;
  onChange: (filterId: string, option: string | null) => void;
}

export function ReportFilterChips({ filters, selection, onChange }: ReportFilterChipsProps) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#616161B2]">
        <SlidersHorizontal className="h-3.5 w-3.5 text-white" aria-hidden="true" />
      </span>
      <span className="text-xs text-white">Filter By:</span>
      {filters.map((filter) => (
        <ReportFilterChip
          key={filter.id}
          {...filter}
          selected={selection[filter.id] ?? null}
          onSelect={(option) => onChange(filter.id, option)}
        />
      ))}
    </div>
  );
}
