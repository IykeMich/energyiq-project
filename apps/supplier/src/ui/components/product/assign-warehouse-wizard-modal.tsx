...
/** Full unit word used in product rows (e.g. "per Litre"). */
const UNIT_WORD: Record<ProductUnit, string> = { L: 'Litre', pcs: 'Piece', Kg: 'Kg' };
  ...
  return (
    ...
    >
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      PRD-{p.sku} · ₦{p.defaultPriceNGN.toLocaleString()} per {UNIT_WORD[p.unit]}
                    </p>
                  </div>
                  {active && (
      ...
      {step === 2 && (
        <>
          {selectedProduct && (
            <p className="text-xs font-semibold text-success bg-success/15 border border-success/40 self-start rounded-full px-4 py-1.5 mb-4 inline-block">
              PRD-{selectedProduct.sku} · {selectedProduct.name}
            </p>
          )}
          ...
          {selectedWarehouses.length > 0 && (
            <div className="mt-4 bg-brand/10 border border-brand rounded-full px-5 py-3 text-center text-sm font-semibold text-brand">
      ...
      {step === 3 && (
        <>
          ...
          <div className="bg-surface-card border border-border-subtle rounded-[20px] p-5 mb-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {selectedProduct?.sku} {selectedProduct ? UNIT_WORD[selectedProduct.unit] : ''}
            </p>
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {selected && (
          <span className="w-5 h-5 rounded-md bg-brand text-brand-foreground flex items-center justify-center shrink-0">
  ...
  return (
    <div className="flex items-center gap-3 mt-6">
      <button
        type="button"
        onClick={onBack}
        className="h-[52px] rounded-full bg-foreground/10 text-foreground font-semibold px-8"
      >
        {backLabel}
      </button>
      <button
        ...
        className={cn(
          'h-[52px] rounded-full bg-brand text-brand-foreground font-semibold px-10 disabled:opacity-50 disabled:cursor-not-allowed',
          primaryClassName ?? 'flex-1',
        )}
      >
        {primaryLabel}
