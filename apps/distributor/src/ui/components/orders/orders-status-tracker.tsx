    ...
    <div className={cn('flex flex-col gap-5 rounded-[15px] bg-[#6161611A] p-5', className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="h-4.25 w-1 rounded-sm bg-[#FBC02D]" />
      ...
      <div className="flex w-full items-start">
        {ORDER_TRACKER_STEPS.map((step, stepIndex) => {
          ...
          const filledBars =
            stepIndex < ORDER_TRACKER_ACTIVE_INDEX ? 3 : stepIndex === ORDER_TRACKER_ACTIVE_INDEX ? 2 : 0;

          return (
            <div key={step.label} className={cn('flex items-start', isLast ? 'flex-initial' : 'flex-1')}>
              {/* Circle + label, top-aligned so the connector can center on the circle. */}
